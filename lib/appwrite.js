import { Client, Account, Databases, Storage, Teams } from 'react-native-appwrite';
// NOTE: This Project ID is for the public Demo/Hackathon environment only.
// It is restricted to the 'com.nomvia.app' bundle signature.
// Do not use for production data.
export const APPWRITE_ENDPOINT = 'https://cloud.appwrite.io/v1';
export const APPWRITE_PROJECT_ID = '6973457f000e977ae601'; // public key only for MVP, it will expire soon.
export const APPWRITE_DB_ID = 'nomvia_db';
export const APPWRITE_COLLECTION_USERS = 'users';
export const APPWRITE_COLLECTION_POSTS = 'posts';
export const APPWRITE_COLLECTION_COMMENTS = 'comments';
export const APPWRITE_COLLECTION_LIKES = 'likes';
export const APPWRITE_COLLECTION_MATCHES = 'matches';
export const APPWRITE_COLLECTION_SWIPES = 'swipes';
export const APPWRITE_COLLECTION_FOLLOWERS = 'followers';
export const APPWRITE_COLLECTION_REQUESTS = 'requests';
export const APPWRITE_BUCKET_ID = 'nomvia_storage';

const client = new Client();

client
    .setEndpoint(APPWRITE_ENDPOINT) 
    .setProject(APPWRITE_PROJECT_ID) 
    .setPlatform('com.nomvia.app'); 

export { client };

export const account = new Account(client);
export const databases = new Databases(client);
export const storage = new Storage(client);
export const teams = new Teams(client);



